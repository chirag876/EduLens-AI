...
#    - Converts the student's question into an embedding vector using the
#      shared embedding model.
#    - The same embedding model used during ingestion is used here so that the
#      question and stored document chunks exist in the same embedding space.
#
# 2. Multi-Source Semantic Search
#    - Searches the PDF and video collections for relevant content.
#    - By default, both collections are searched.
#    - The caller can optionally restrict retrieval to either PDF or Video
#      content using the source_type parameter.
#
# 3. Top-K Retrieval
#    - Retrieves up to TOP_K results from each searched collection.
#    - The current configuration retrieves the top 5 chunks from each source.
#
# 4. Metadata Filtering
#    - Applies source-specific metadata filters when searching the
#      collections.
#    - This ensures that PDF searches target PDF content and video searches
#      target video content.
#
# 5. Result Merging and Ranking
#    - Results from the searched collections are combined into a single list.
#    - The merged results are sorted by ChromaDB distance.
#    - A lower distance indicates that the retrieved chunk is more relevant
#      to the student's question.
#
# 6. Graceful Handling of Empty Collections
#    - Individual collection search failures do not cause the complete
#      retrieval operation to fail.
#    - _search_collection returns an empty list when a collection is empty,
#      unavailable, or cannot be queried.
#    - This allows the application to continue gracefully when no content has
#      been ingested yet.
#
# 7. Input Validation and Error Handling
#    - Rejects empty or whitespace-only questions before performing retrieval.
#    - Unexpected retrieval failures are converted into application-level
